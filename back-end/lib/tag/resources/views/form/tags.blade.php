...
@php($model = $type ?? \Newnet\Tag\Models\Tag::class)
...
<div class="form-group row component-{{ $name }}">
    ...
    <div class="col-12">
        ...
        >
            ...
            @endforeach
            @foreach(get_another_tag(object_get($item, $name)->pluck('id')->toArray(), $model) as $tagAnotherItem)
                    <option value="{{ $tagAnotherItem->id ?? $tagAnotherItem }}">
                        {{ $tagAnotherItem->name ?? $tagAnotherItem }}
                    </option>
            @endforeach
        </select>
