<div class="form-group row component-{{ $name }}">
    <label for="{{ $name }}" class="col-12 col-form-label font-weight-600">{{ $label }} {!! isset($required) ? '<span style="color: red">*</span>' : '' !!}</label>
    <div class="col-12">
        ...
        >
            @if(!empty($allowClear ?? true))
                <option @if (!empty($option['type'])) type="{{ $option['type'] }}" @endif value="">{{ $placeholder ?? "--- {$label} ---" }}</option>
            @endif
            @foreach($options as $option)
                <option @if (!empty($option['type'])) data-type="{{ $option['type'] }}" @endif value="{{ $option['value'] }}"
